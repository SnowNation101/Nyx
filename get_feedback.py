...
def index():
    ...
    corpus_path = "baselines/text-only/t2t_train_corpus.json"
    with open(corpus_path, "r") as f:
        corpus = json.load(f)

    index_path = "baselines/text-only/index/nyx_train.faiss"
    embeddings = []
...
# ===============RETRIEVAL================
def retrieval():
    print("Start retrieving documents...")
    text_index = faiss.read_index("baselines/text-only/index/nyx_train.faiss")
    text_corpus = json.load(open("baselines/text-only/t2t_train_corpus.json", "r"))
    ...
    for subset in text_subsets:
        ...
        for item in tqdm(dataset, desc=f"Retrieving {subset}"):
            ...
            item["retrieved_docs"] = retrieve(text_index, text_corpus, text)
...
def filter():
    dataset = json.load(open("outputs/feedback/train_hardneg.json", "r"))
    filtered_dataset = []
    for item in dataset:
        cnt = 0
        for image in item['qry_image_path']:
            cnt += 1
        for image in item['pos_image_path']:
            cnt += 1
        for image in item['neg_image_path'][0]:
            cnt += 1
        if cnt < 8:
            filtered_dataset.append(item)
    # Save the filtered dataset to a new file
    with open("/fs/archive/share/mm_datasets/NyxQA/filtered_train_hardneg.json", "w") as f:
        json.dump(filtered_dataset, f, indent=4)

    dataset = json.load(open("outputs/feedback/final_feedback.json", "r"))
    filtered_dataset = []
    for item in dataset:
        cnt = 0
        for image in item['qry_image_path']:
    ...
    # Save the filtered dataset to a new file
    with open("outputs/feedback/filtered_feedback.json", "w") as f:
        json.dump(filtered_dataset, f, indent=4)
