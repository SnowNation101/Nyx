...
# ==============configurations================
generated_dir = "generated/internvl"
...
for item in tqdm(test_data, desc="Generating answers for test"):
    ...
    prompt = prompt.replace(IMAGE_TOKEN, INTERNVL_IMAGE_TOKEN)

    if images:
        pixel_values_list = [load_image(os.path.join(image_dir, img)).to(torch.bfloat16).cuda() for img in images]
