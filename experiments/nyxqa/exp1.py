...
parser.add_argument("--ckpt_path", type=str, required=True, help="Path to the LoRA checkpoint")
...
ckpt_name = os.path.basename(os.path.normpath(args.ckpt_path))
index_path = f"index/nyx_{ckpt_name}.faiss"
index_dir = "index"
retrieved_dir = f"retrieved/nyx_{ckpt_name}"
generated_dir = f"generated/nyx_{ckpt_name}"
retrieve_top_k = 10
generate_top_k = 1
...
def retrieval():
    ...
    if not os.path.exists(index_path):
        ...
        for item in tqdm(corpus, desc="Indexing"):
            text = item['text'].replace(IMAGE_TOKEN, QWEN_IMAGE_TOKEN)
...
def generation():
    ...
    for item in tqdm(test_data, desc="Generating answers for test"):
        ...
        docs = [doc['text'].replace(IMAGE_TOKEN, QWEN_IMAGE_TOKEN) for doc in retrieved_docs[:generate_top_k]]
