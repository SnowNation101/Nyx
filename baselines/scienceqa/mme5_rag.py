...
if not os.path.exists(os.path.join(index_dir, "mmE5_lecture.faiss")):
    embeddings = []
    ...
    faiss.write_index(lecture_index, os.path.join(index_dir, "mmE5_lecture.faiss"))
else:
    lecture_index = faiss.read_index(os.path.join(index_dir, "mmE5_lecture.faiss"))

# index example Q&A corpus
if not os.path.exists(os.path.join(index_dir, "mmE5_example_qa.faiss")):
    embeddings = []
    for item in tqdm(example_qa_corpus, desc="Indexing example Q&A corpus"):
        text = item['text']
        if item['image']:
    ...
    faiss.write_index(example_qa_index, os.path.join(index_dir, "mmE5_example_qa.faiss"))
else:
    example_qa_index = faiss.read_index(os.path.join(index_dir, "mmE5_example_qa.faiss"))
